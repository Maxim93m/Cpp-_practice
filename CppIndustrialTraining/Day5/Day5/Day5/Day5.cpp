...
int main() {
    ...
    Compressor c1 = inputCompressor(); // Ввод параметров компрессора
    ...
    printTable(warnings, alarms, c1.name, is_running, c1.avgTemp, c1.press, c1.vibr, c1.hours, c1.filterState);
    // Отчёт по параметрам системы
    createReport(warnings, alarms, c1.name, is_running, c1.avgTemp, c1.press, c1.vibr, c1.hours, c1.filterState);
...
//<---------------------- ОПИСАНИЕ ФУНКЦИЙ -------------------------> //
// Проверка значений параметров
void checkCompressor(Compressor& c, int& warnings, int& alarms) {
    float sumTemp = 0;
    for (int i = 0; i < c.tempSensors.size(); i++) {
        checkValueTemperature(c.tempSensors[i], warnings, alarms, i); // Tемпературa масла
        sumTemp += c.tempSensors[i];
    }
    c.avgTemp = sumTemp / c.tempSensors.size();
    checkValuePressure(c.press, warnings, alarms); // Давление воздуха
    checkValueVibration(c.vibr, warnings, alarms); // Уровень вибраций
    checkValueMotoHours(c.hours, warnings, alarms); // Моточасы
    checkStateFilter(c.filterState, alarms); // Состояние фильтра 
...
Compressor inputCompressor() {
    ...
    cm.tempSensors.resize(count_temp_sensors);
    for (int i = 0; i < cm.tempSensors.size(); i++) {
        std::string temp_sensors_prompt = "Введите значение температурного датчика TE" + std::to_string(i + 1) + ": ";
        cm.tempSensors[i] = getFloatInput(temp_sensors_prompt);
    }
    cm.press = getIntInput("Введите значение давления воздуха: ");
    cm.vibr = getFloatInput("Введите значение уровня вибрации: ");
    cm.hours = getIntInput("Введите значение моточасов: ");
    int filter_input = getFilterInput("Введите значение состояния фильтра: ");
    cm.filterState = (filter_input == 1);
    // Очистка буфера
